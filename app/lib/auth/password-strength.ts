 ...
 *
 * It moved out of `app/lib/e2ee/flows/` in M191 with the encrypted layer. The
 * floor it reads now comes from `password-rule.ts`, which is where this
 * installation states its one password rule.
 */
/**
 ...
 * `MIN_PASSWORD_LENGTH`), enforced by `validatePassword`. This
