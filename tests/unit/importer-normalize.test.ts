/**
 * `normalizeLemma` and `tokenize`, the two functions the dictionary's identity
 * rests on.
 *
 * WHY THESE TWO DESERVE THEIR OWN FILE
 *   `normalizeLemma` decides when two written forms are the same word. It wrote
 *   every value in `headwords.lemma_normalized` on import, and it produces every
 *   value a lookup compares against. A change here does not fail: it silently
 *   moves rows out of reach of the queries that should find them.
 *
 *   `tokenize` is the same function applied to a sentence. The Tatoeba
 *   attachment join sends its output into SQL to be matched against the stored
 *   column with a plain `=`. So the two sides of that equality are only sound
 *   while every token `tokenize` emits is already in stored form. That is the
...
import { normalizeLemma, tokenize } from '../../cli/lib/importers/normalize';
...
describe('tokenize', () => {
  it('splits on punctuation, keeps hyphens and apostrophes inside a word', () => {
    ...
    assert.deepEqual(tokenize("Don't stop, don't! E-mail me."), [
      "don't",
      'stop',
      'e-mail',
      'me',
  ...
  it('deduplicates while keeping first-appearance order', () => {
    ...
    assert.deepEqual(tokenize('Ein E-Mail-Konto, ein Konto.'), [
      'ein',
      'e-mail-konto',
      'konto',
    ]);
  ...
  it('drops empty pieces, including from a string with no words at all', () => {
    ...
    assert.deepEqual(tokenize(''), []);
    assert.deepEqual(tokenize('¡Hola!'), ['hola']);
  });
  ...
  it('emits only tokens that are already in stored form', () => {
    ...
    // with a plain `=`, and that column was written by `normalizeLemma`. So a
    // token is findable only when it is a fixed point of `normalizeLemma`:
    // `normalizeLemma(token) === token`. If tokenize ever emitted a form that
    // normalizing would change, the join would return fewer rows, no error
    // would be raised, and nothing would tell us the matches were lost.
    const sentences = [
...
describe('a known limitation', () => {
  it('leaves Turkish dotted and dotless i to a later spec', () => {
    // This is documentation of what we do NOT do, not a behaviour we want.
    //
    // In Turkish the lowercase of I is ı, and the lowercase of İ is i.
    // JavaScript's locale-free `toLowerCase` maps both capitals towards i, so
    // the uppercase and lowercase spellings of one Turkish word normalize to
    // two different stored forms, and a lookup on one will not find the other.
    assert.equal(normalizeLemma('ışık'), 'ısık');
    assert.equal(normalizeLemma('IŞIK'), 'isik');
    assert.notEqual(
      normalizeLemma('IŞIK'),
      normalizeLemma('ışık'),
      'if these now agree, Turkish casing was implemented and this case should become a real assertion',
    );

    // İ happens to come out right, and by accident rather than by rule: it
    // decomposes to I plus a combining dot, and the dot is stripped as a mark.
    assert.equal(normalizeLemma('İstanbul'), 'istanbul');
    assert.equal(normalizeLemma('Istanbul'), 'istanbul');
  });
});
