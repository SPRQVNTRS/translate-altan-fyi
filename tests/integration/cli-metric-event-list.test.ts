/**
 * Integration test: `cli metric-event list --org=default`
 *
...
describe('cli metric-event list', () => {
  it('lists metric events and returns data field', { skip: !TEST_API_KEY ? 'TEST_API_KEY not set' : false }, () => {
    const result = spawnSync(
      'node_modules/.bin/tsx',
      ['cli/index.ts', '--remote', REMOTE_URL, 'metric-event', 'list', '--org=default', '--format=json'],
      {
        encoding: 'utf8',
    ...
    // Response contains a "data" field — check stdout for the key
    assert.ok(
