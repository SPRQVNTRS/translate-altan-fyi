/**
 * Integration test: `cli user list --format=json`
 *
...
describe('cli user list', () => {
  it('lists users as superadmin and returns data field', { skip: !TEST_API_KEY ? 'TEST_API_KEY not set' : false }, () => {
    const result = spawnSync(
      'node_modules/.bin/tsx',
      ['cli/index.ts', '--remote', REMOTE_URL, 'user', 'list', '--format=json'],
      {
        encoding: 'utf8',
